...
var _ = Describe("Describe Command", func() {
	...
	Describe("Run", func() {
		It("Calls the pkg/svcat libs RetrievePlanByName with cluster scope options", func() {
			...
			planToReturn := &v1beta1.ClusterServicePlan{Spec: v1beta1.ClusterServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planName}}}
			classToReturn := &v1beta1.ClusterServiceClass{Spec: v1beta1.ClusterServiceClassSpec{CommonServiceClassSpec: v1beta1.CommonServiceClassSpec{ExternalName: className}}}
			...
			cmd := describeCmd{Namespaced: &command.Namespaced{Context: svcattest.NewContext(outputBuffer, fakeApp)}, Scoped: command.NewScoped()}
			...
			Expect(scopeArg).To(Equal(servicecatalog.ScopeOptions{Scope: servicecatalog.ClusterScope}))
		...
		It("Calls the pkg/svcat libs RetrievePlanByName with namespace scope options", func() {
			...
			className := "clusterclass"
			planToReturn := &v1beta1.ServicePlan{ObjectMeta: metav1.ObjectMeta{Namespace: namespaceName}, Spec: v1beta1.ServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planName}}}
			classToReturn := &v1beta1.ClusterServiceClass{Spec: v1beta1.ClusterServiceClassSpec{CommonServiceClassSpec: v1beta1.CommonServiceClassSpec{ExternalName: className}}}
			outputBuffer := &bytes.Buffer{}
			fakeApp, _ := svcat.NewApp(nil, nil, "default")
			fakeSDK := new(servicecatalogfakes.FakeSvcatClient)
			...
			nameArg, scopeArg := fakeSDK.RetrievePlanByNameArgsForCall(0)
			Expect(nameArg).To(Equal(planName))
			Expect(scopeArg).To(Equal(servicecatalog.ScopeOptions{Scope: servicecatalog.NamespaceScope, Namespace: namespaceName}))
			output := outputBuffer.String()
			Expect(output).To(ContainSubstring(planName))
			Expect(output).To(ContainSubstring(className))
		})
