		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, name := range tc.fakeClusterPlans {
				if strings.Contains(name, "bad") {
					svcatClient.PrependReactor("list", "clusterserviceplans", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
...
var _ = Describe("Get Plans Command", func() {
	...
	Describe("Run", func() {
		It("Calls the pkg/svcat libs RetrievePlans with namespace scope and current namespace", func() {
			...
			planToReturn := &v1beta1.ServicePlan{ObjectMeta: metav1.ObjectMeta{Namespace: planNamespace}, Spec: v1beta1.ServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planName}}}
			...
			Expect(scopeArg).To(Equal(servicecatalog.ScopeOptions{Scope: servicecatalog.NamespaceScope, Namespace: planNamespace}))
		...
		It("Calls the pkg/svcat libs RetrievePlans with namespace scope and all namespaces", func() {
			...
			planOneToReturn := &v1beta1.ServicePlan{ObjectMeta: metav1.ObjectMeta{Namespace: planOneNamespace}, Spec: v1beta1.ServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planOneName}}}
			planTwoToReturn := &v1beta1.ServicePlan{ObjectMeta: metav1.ObjectMeta{Namespace: planTwoNamespace}, Spec: v1beta1.ServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planTwoName}}}
			outputBuffer := &bytes.Buffer{}
			fakeApp, _ := svcat.NewApp(nil, nil, "default")
			fakeSDK := new(servicecatalogfakes.FakeSvcatClient)
			...
			Expect(err).NotTo(HaveOccurred())
			_, scopeArg := fakeSDK.RetrievePlansArgsForCall(0)
			Expect(scopeArg).To(Equal(servicecatalog.ScopeOptions{Scope: servicecatalog.NamespaceScope, Namespace: ""}))
			output := outputBuffer.String()
		...
		It("Calls the pkg/svcat libs RetrievePlans with all scope and current namespaces", func() {
			...
			planOneToReturn := &v1beta1.ClusterServicePlan{Spec: v1beta1.ClusterServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planOneName}}}
			planTwoToReturn := &v1beta1.ServicePlan{ObjectMeta: metav1.ObjectMeta{Namespace: planTwoNamespace}, Spec: v1beta1.ServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planTwoName}}}
			outputBuffer := &bytes.Buffer{}
			fakeApp, _ := svcat.NewApp(nil, nil, "default")
			fakeSDK := new(servicecatalogfakes.FakeSvcatClient)
			...
			Expect(err).NotTo(HaveOccurred())
			_, scopeArg := fakeSDK.RetrievePlansArgsForCall(0)
			Expect(scopeArg).To(Equal(servicecatalog.ScopeOptions{Scope: servicecatalog.AllScope, Namespace: planTwoNamespace}))
			output := outputBuffer.String()
			Expect(output).To(ContainSubstring(planOneName))
			Expect(output).To(ContainSubstring(planTwoName))
		})
