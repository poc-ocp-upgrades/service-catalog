...
var _ = Describe("Register Command", func() {
	...
	Describe("Run", func() {
		It("Calls the pkg/svcat libs methods to retrieve all classes and plans and prints output to the user", func() {
			...
			planGUID3 := "banana54"
			classToReturn := &v1beta1.ClusterServiceClass{ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: classGUID}, Spec: v1beta1.ClusterServiceClassSpec{CommonServiceClassSpec: v1beta1.CommonServiceClassSpec{Description: classDescription, ExternalName: className}}}
			classToReturn2 := &v1beta1.ClusterServiceClass{ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: classGUID2}, Spec: v1beta1.ClusterServiceClassSpec{CommonServiceClassSpec: v1beta1.CommonServiceClassSpec{Description: classDescription2, ExternalName: className2}}}
			planToReturn := &v1beta1.ClusterServicePlan{ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: planGUID}, Spec: v1beta1.ClusterServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planName}, ClusterServiceClassRef: v1beta1.ClusterObjectReference{Name: classGUID}}}
			planToReturn2 := &v1beta1.ClusterServicePlan{ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: planGUID2}, Spec: v1beta1.ClusterServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planName2}, ClusterServiceClassRef: v1beta1.ClusterObjectReference{Name: classGUID}}}
			planToReturn3 := &v1beta1.ClusterServicePlan{ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: planGUID3}, Spec: v1beta1.ClusterServicePlanSpec{CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{ExternalName: planName3}, ClusterServiceClassRef: v1beta1.ClusterObjectReference{Name: classGUID2}}}
			...
			cmd := MarketplaceCmd{Namespaced: &command.Namespaced{Context: svcattest.NewContext(outputBuffer, fakeApp)}, Formatted: command.NewFormatted()}
			cmd.Namespace = namespace
			err := cmd.Run()
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeSDK.RetrieveClassesCallCount()).To(Equal(1))
			scopeOpts := fakeSDK.RetrieveClassesArgsForCall(0)
			Expect(scopeOpts).To(Equal(servicecatalog.ScopeOptions{Scope: servicecatalog.AllScope, Namespace: namespace}))
