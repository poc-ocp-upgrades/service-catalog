...
var _ = Describe("Register Command", func() {
	...
	Describe("Validate", func() {
		...
		It("errors if both basic-secret and bearer-secret are provided", func() {
			...
			cmd := RegisterCmd{BasicSecret: basicSecret, BearerSecret: bearerSecret}
	...
	Describe("Run", func() {
		...
		It("Calls the pkg/svcat libs Register method with the passed in variables and prints output to the user", func() {
			...
			cmd := RegisterCmd{BasicSecret: basicSecret, BrokerName: brokerName, CAFile: certFile, ClassRestrictions: classRestrictions, Namespaced: command.NewNamespaced(cxt), PlanRestrictions: planRestrictions, RelistBehavior: relistBehavior, RelistDuration: relistDuration, Scoped: command.NewScoped(), SkipTLS: skipTLS, URL: brokerURL, Waitable: command.NewWaitable()}
			...
			opts := servicecatalog.RegisterOptions{BasicSecret: basicSecret, CAFile: certFile, ClassRestrictions: classRestrictions, Namespace: namespace, PlanRestrictions: planRestrictions, RelistBehavior: relistBehaviorConst, RelistDuration: metaRelistDuration, SkipTLS: skipTLS}
		...
		It("Passes in the bearer secret", func() {
			...
			brokerToReturn.Spec.AuthInfo.Bearer = &v1beta1.ClusterBearerTokenAuthConfig{SecretRef: &v1beta1.ObjectReference{Name: bearerSecret}}
			outputBuffer := &bytes.Buffer{}
			fakeApp, _ := svcat.NewApp(nil, nil, namespace)
			fakeSDK := new(servicecatalogfakes.FakeSvcatClient)
			fakeSDK.RegisterReturns(brokerToReturn, nil)
			fakeApp.SvcatClient = fakeSDK
			cxt := svcattest.NewContext(outputBuffer, fakeApp)
			cmd := RegisterCmd{BearerSecret: bearerSecret, BrokerName: brokerName, Namespaced: command.NewNamespaced(cxt), Scoped: command.NewScoped(), Waitable: command.NewWaitable(), URL: brokerURL}
			cmd.Namespaced.ApplyNamespaceFlags(&pflag.FlagSet{})
			cmd.Waitable.ApplyWaitFlags()
			err := cmd.Run()
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(returnedName).To(Equal(brokerName))
			Expect(returnedURL).To(Equal(brokerURL))
			opts := servicecatalog.RegisterOptions{Namespace: namespace, BearerSecret: bearerSecret}
			Expect(*returnedOpts).To(Equal(opts))
			output := outputBuffer.String()
			Expect(output).To(ContainSubstring(brokerName))
			Expect(output).To(ContainSubstring(brokerURL))
		...
		It("Calls the SDK's WaitForBroker method with the passed in interval and timeout when Wait==true", func() {
			...
			fakeApp.SvcatClient = fakeSDK
			cxt := svcattest.NewContext(outputBuffer, fakeApp)
			cmd := RegisterCmd{BrokerName: brokerName, Namespaced: command.NewNamespaced(cxt), Scoped: command.NewScoped(), Waitable: command.NewWaitable(), URL: brokerURL}
			cmd.Wait = true
			cmd.Namespaced.ApplyNamespaceFlags(&pflag.FlagSet{})
			cmd.Waitable.ApplyWaitFlags()
			...
			Expect(returnedName).To(Equal(brokerName))
			Expect(returnedURL).To(Equal(brokerURL))
			opts := servicecatalog.RegisterOptions{Namespace: namespace}
			Expect(*returnedOpts).To(Equal(opts))
