...
func createMultipleItemStorage() NamespacedStorage {
	...
	storage := make(NamespacedStorage)
	storage.Set(ns1, tipe1, name1, &servicecatalog.ClusterServiceBroker{})
	storage.Set(ns1, tipe1, name2, &servicecatalog.ClusterServiceBroker{})
	storage.Set(ns1, tipe1, name3, &servicecatalog.ClusterServiceBroker{})
	storage.Set(ns2, tipe1, name1, &servicecatalog.ClusterServiceBroker{})
	storage.Set(ns2, tipe1, name2, &servicecatalog.ClusterServiceBroker{})
	return storage
}
...
func TestGetItem(t *testing.T) {
	...
	testCases := []struct {
		...
		url		string
		expectedStatus	int
	}{{"Empty Storage", make(NamespacedStorage), NewWatcher(), newResponseWriter(), fmt.Sprintf("/apis/servicecatalog.k8s.io/v1beta1/namespaces/%s/%s/%s", ns1, tipe1, name1), http.StatusNotFound}, {"One Item in storage", createSingleItemStorage(), NewWatcher(), newResponseWriter(), fmt.Sprintf("/apis/servicecatalog.k8s.io/v1beta1/namespaces/%s/%s/%s", ns1, tipe1, name1), http.StatusOK}}
...
func TestGetItems(t *testing.T) {
	...
	testCases := []struct {
		...
		url		string
		expectedStatus	int
	}{{"Empty Storage", make(NamespacedStorage), NewWatcher(), newResponseWriter(), fmt.Sprintf("/apis/servicecatalog.k8s.io/v1beta1/namespaces/%v/clusterservicebrokers", ns1), http.StatusOK}, {"Multiple Items", createMultipleItemStorage(), NewWatcher(), newResponseWriter(), fmt.Sprintf("/apis/servicecatalog.k8s.io/v1beta1/namespaces/%v/clusterservicebrokers", ns1), http.StatusOK}}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			request, err := http.NewRequest("GET", tc.url, nil)
			if err != nil {
...
func TestCreateItem(t *testing.T) {
	...
	}{{"Create Item (empty storage)", make(NamespacedStorage), NewWatcher(), newResponseWriter(), fmt.Sprintf("/apis/servicecatalog.k8s.io/v1beta1/namespaces/%s/%s", ns1, tipe1), &servicecatalog.ClusterServiceBroker{ObjectMeta: metav1.ObjectMeta{Name: name1}, TypeMeta: metav1.TypeMeta{Kind: "ClusterServiceBroker", APIVersion: "servicecatalog.k8s.io/v1beta1"}}, http.StatusCreated, 1}, {"Create Item (non-empty storage)", createMultipleItemStorage(), NewWatcher(), newResponseWriter(), fmt.Sprintf("/apis/servicecatalog.k8s.io/v1beta1/namespaces/%s/%s", ns1, tipe1), &servicecatalog.ClusterServiceBroker{TypeMeta: metav1.TypeMeta{Kind: "ClusterServiceBroker", APIVersion: "servicecatalog.k8s.io/v1beta1"}}, http.StatusCreated, 2}}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
