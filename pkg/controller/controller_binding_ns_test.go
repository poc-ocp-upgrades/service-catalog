...
func TestReconcileServiceBindingWithParametersNamespacedRefs(t *testing.T) {
	...
	fakeKubeClient, fakeCatalogClient, fakeBrokerClient, testController, sharedInformers := newTestController(t, fakeosb.FakeClientConfiguration{BindReaction: &fakeosb.BindReaction{Response: &osb.BindResponse{Credentials: map[string]interface{}{"a": "b", "c": "d"}}}})
	...
	assertBind(t, brokerActions[0], &osb.BindRequest{BindingID: testServiceBindingGUID, InstanceID: testServiceInstanceGUID, ServiceID: testServiceClassGUID, PlanID: testServicePlanGUID, AppGUID: strPtr(testNamespaceGUID), Parameters: map[string]interface{}{"args": []interface{}{"first-arg", "second-arg"}, "name": "test-param"}, BindResource: &osb.BindResource{AppGUID: strPtr(testNamespaceGUID)}, Context: testContext})
...
func TestReconcileServiceBindingDeleteNamespacedRefs(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			brokerActions := fakeBrokerClient.Actions()
			assertNumberOfBrokerActions(t, brokerActions, 1)
			assertUnbind(t, brokerActions[0], &osb.UnbindRequest{BindingID: testServiceBindingGUID, InstanceID: testServiceInstanceGUID, ServiceID: testServiceClassGUID, PlanID: testServicePlanGUID})
			kubeActions := fakeKubeClient.Actions()
...
func TestPollServiceBindingNamespacedRefs(t *testing.T) {
	...
	validatePollBindingLastOperationAction := func(t *testing.T, actions []fakeosb.Action) {
		...
		assertPollBindingLastOperation(t, actions[0], &osb.BindingLastOperationRequest{InstanceID: testServiceInstanceGUID, BindingID: testServiceBindingGUID, ServiceID: strPtr(testServiceClassGUID), PlanID: strPtr(testServicePlanGUID), OperationKey: &operationKey})
	}
...
func TestReconcileServiceBindingAsynchronousUnbindNamespacedRefs(t *testing.T) {
	...
	brokerActions := fakeServiceBrokerClient.Actions()
	assertNumberOfBrokerActions(t, brokerActions, 1)
	assertUnbind(t, brokerActions[0], &osb.UnbindRequest{BindingID: testServiceBindingGUID, InstanceID: testServiceInstanceGUID, ServiceID: testServiceClassGUID, PlanID: testServicePlanGUID, AcceptsIncomplete: true})
	assertDeleteSecretAction(t, fakeKubeClient.Actions(), binding.Spec.SecretName)
	actions := fakeCatalogClient.Actions()
	assertNumberOfActions(t, actions, 1)
	updatedServiceBinding := assertUpdateStatus(t, actions[0], binding).(*v1beta1.ServiceBinding)
