...
func TestReconcileServiceClassRemovedFromCatalog(t *testing.T) {
	...
	}{{name: "not removed from catalog", serviceClass: getTestServiceClass(), shouldError: false}, {name: "removed from catalog, instances left", serviceClass: getRemovedServiceClass(), instances: []v1beta1.ServiceInstance{*getTestServiceInstance()}, shouldError: false, catalogActionsCheckFunc: func(t *testing.T, actions []clientgotesting.Action) {
		listRestrictions := clientgotesting.ListRestrictions{Labels: labels.Everything(), Fields: fields.OneTermEqualSelector("spec.serviceClassRef.name", "scguid")}
		assertNumberOfActions(t, actions, 1)
		assertList(t, actions[0], &v1beta1.ServiceInstance{}, listRestrictions)
	}}, {name: "removed from catalog, no instances left", serviceClass: getRemovedServiceClass(), instances: nil, shouldError: false, catalogActionsCheckFunc: func(t *testing.T, actions []clientgotesting.Action) {
		listRestrictions := clientgotesting.ListRestrictions{Labels: labels.Everything(), Fields: fields.OneTermEqualSelector("spec.serviceClassRef.name", "scguid")}
		assertNumberOfActions(t, actions, 2)
		assertList(t, actions[0], &v1beta1.ServiceInstance{}, listRestrictions)
		assertDelete(t, actions[1], getRemovedServiceClass())
	}}, {name: "removed from catalog, no instances left, delete fails", serviceClass: getRemovedServiceClass(), instances: nil, shouldError: true, catalogClientPrepFunc: func(client *fake.Clientset) {
