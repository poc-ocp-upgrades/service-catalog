...
func listServiceClassesReactor(classes []v1beta1.ServiceClass) (string, string, clientgotesting.ReactionFunc) {
	...
	return "list", "serviceclasses", func(action clientgotesting.Action) (bool, runtime.Object, error) {
		return true, &v1beta1.ServiceClassList{Items: classes}, nil
	}
}
...
func TestReconcileServiceBrokerDelete(t *testing.T) {
	...
	}{{name: "no auth", authInfo: nil, secret: nil}, {name: "basic auth", authInfo: getTestBrokerBasicAuthInfo(), secret: getTestBasicAuthSecret()}, {name: "bearer auth", authInfo: getTestBrokerBearerAuthInfo(), secret: getTestBearerAuthSecret()}}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			listRestrictions := clientgotesting.ListRestrictions{Labels: labels.Everything(), Fields: fields.OneTermEqualSelector("spec.serviceBrokerName", broker.Name)}
			...
			expectedEvent := normalEventBuilder(successServiceBrokerDeletedReason).msg("The servicebroker test-servicebroker was deleted successfully.")
...
func TestReconcileServiceClassFromServiceBrokerCatalog(t *testing.T) {
	...
	}{{name: "new class", newServiceClass: getTestServiceClass(), shouldError: false, catalogActionsCheckFunc: func(t *testing.T, actions []clientgotesting.Action) {
		assertNumberOfActions(t, actions, 1)
		assertCreate(t, actions[0], getTestServiceClass())
	}}, {name: "exists, but for a different broker", newServiceClass: getTestServiceClass(), existingServiceClass: getTestServiceClass(), listerServiceClass: func() *v1beta1.ServiceClass {
		p := getTestServiceClass()
		p.Spec.ServiceBrokerName = "something-else"
		return p
	}(), shouldError: true, errText: strPtr(`ServiceBroker "test-servicebroker": ServiceClass "test-serviceclass" already exists for Broker "something-else"`)}, {name: "class update", newServiceClass: updatedClass(), existingServiceClass: getTestServiceClass(), shouldError: false, catalogActionsCheckFunc: func(t *testing.T, actions []clientgotesting.Action) {
		assertNumberOfActions(t, actions, 1)
		assertUpdate(t, actions[0], updatedClass())
	}}, {name: "class update - failure", newServiceClass: updatedClass(), existingServiceClass: getTestServiceClass(), catalogClientPrepFunc: func(client *fake.Clientset) {
...
func TestReconcileServicePlanFromServiceBrokerCatalog(t *testing.T) {
	...
	cases := []struct {
		...
		catalogClientPrepFunc	func(*fake.Clientset)
		catalogActionsCheckFunc	func(t *testing.T, actions []clientgotesting.Action)
	}{{name: "new plan", newServicePlan: getTestServicePlan(), shouldError: false, catalogActionsCheckFunc: func(t *testing.T, actions []clientgotesting.Action) {
		assertNumberOfActions(t, actions, 1)
		assertCreate(t, actions[0], getTestServicePlan())
	}}, {name: "exists, but for a different broker", newServicePlan: getTestServicePlan(), existingServicePlan: getTestServicePlan(), listerServicePlan: func() *v1beta1.ServicePlan {
		p := getTestServicePlan()
		p.Spec.ServiceBrokerName = "something-else"
		return p
	}(), shouldError: true, errText: strPtr(`ServiceBroker "test-servicebroker": ServicePlan "test-serviceplan" already exists for Broker "something-else"`)}, {name: "plan update", newServicePlan: updatedPlan(), existingServicePlan: getTestServicePlan(), shouldError: false, catalogActionsCheckFunc: func(t *testing.T, actions []clientgotesting.Action) {
		assertNumberOfActions(t, actions, 1)
		assertUpdate(t, actions[0], updatedPlan())
	}}, {name: "plan update - failure", newServicePlan: updatedPlan(), existingServicePlan: getTestServicePlan(), catalogClientPrepFunc: func(client *fake.Clientset) {
