...
func TestServiceBrokerUpdate(t *testing.T) {
	...
	}{{name: "no spec change", older: serviceBrokerWithOldSpec(), newer: serviceBrokerWithOldSpec(), shouldGenerationIncrement: false}, {name: "spec change", older: serviceBrokerWithOldSpec(), newer: serviceBrokerWithNewSpec(), shouldGenerationIncrement: true}}
	creatorUserName := "creator"
	createContext := sctestutil.ContextWithUserName(creatorUserName)
	for i := range cases {
