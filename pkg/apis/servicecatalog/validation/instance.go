...
var validServiceInstanceOperations = map[sc.ServiceInstanceOperation]bool{sc.ServiceInstanceOperation(""): true, sc.ServiceInstanceOperationProvision: true, sc.ServiceInstanceOperationUpdate: true, sc.ServiceInstanceOperationDeprovision: true}
...
var validServiceInstanceDeprovisionStatuses = map[sc.ServiceInstanceDeprovisionStatus]bool{sc.ServiceInstanceDeprovisionStatusNotRequired: true, sc.ServiceInstanceDeprovisionStatusRequired: true, sc.ServiceInstanceDeprovisionStatusSucceeded: true, sc.ServiceInstanceDeprovisionStatusFailed: true}
...
func internalValidateServiceInstance(instance *sc.ServiceInstance, create bool) field.ErrorList {
	...
	allErrs = append(allErrs, apivalidation.ValidateObjectMeta(&instance.ObjectMeta, true, validateServiceInstanceName, field.NewPath("metadata"))...)
...
func validateServiceInstanceUpdate(instance *sc.ServiceInstance) field.ErrorList {
	...
	if instance.Status.CurrentOperation != "" {
		...
		if instance.Status.CurrentOperation != sc.ServiceInstanceOperationDeprovision {
			...
			clusterUnset := instance.Spec.ClusterServicePlanRef == nil && (instance.Status.ExternalProperties == nil || instance.Status.ExternalProperties.ClusterServicePlanExternalID == "")
			nsUnset := instance.Spec.ServicePlanRef == nil && (instance.Status.ExternalProperties == nil || instance.Status.ExternalProperties.ServicePlanExternalID == "")
...
func validatePlanReference(p *sc.PlanReference, fldPath *field.Path) field.ErrorList {
	...
	cases := []struct {
		...
		ns	string
		field	string
	}{{p.ClusterServiceClassExternalName, p.ServiceClassExternalName, "serviceClassExternalName"}, {p.ClusterServiceClassExternalID, p.ServiceClassExternalID, "serviceClassExternalID"}, {p.ClusterServiceClassName, p.ServiceClassName, "serviceClassName"}, {p.ClusterServicePlanExternalName, p.ServicePlanExternalName, "servicePlanExternalName"}, {p.ClusterServicePlanExternalID, p.ServicePlanExternalID, "servicePlanExternalID"}, {p.ClusterServicePlanName, p.ServicePlanName, "servicePlanName"}}
...
func validateScopedPlanRef(h scopedRefHelper, p *sc.PlanReference, fldPath *field.Path) field.ErrorList {
	...
	if (b2i(externalClassNameSet) + b2i(externalClassIDSet) + b2i(k8sClassSet)) != 1 {
		classSetErrMsg := fmt.Sprintf("exactly one of %s, %s, or %s required", h.classField("ExternalName"), h.classField("ExternalID"), h.classField("Name"))
	...
	if (b2i(externalPlanNameSet) + b2i(externalPlanIDSet) + b2i(k8sPlanSet)) > 1 {
		planSetErrMsg := fmt.Sprintf("exactly one of %s, %s, or %s required", h.planField("ExternalName"), h.planField("ExternalID"), h.planField("Name"))
	...
	if externalClassNameSet {
		for _, msg := range validateCommonServiceClassName(h.externalClassName, false) {
		...
		if !externalPlanNameSet {
			...
			for _, msg := range validateCommonServicePlanName(h.externalPlanName, false) {
		...
		for _, msg := range validateCommonServiceClassName(h.k8sClass, false) {
		...
		if !k8sPlanSet {
			...
			for _, msg := range validateCommonServicePlanName(h.k8sPlan, false) {
