...
func testNoName(client servicecatalogclient.Interface) error {
	...
	if br, e := scClient.ServiceBrokers(ns).Create(&v1beta1.ServiceBroker{}); nil == e {
		return fmt.Errorf("needs a name (%s)", br.Name)
	}
	if sc, e := scClient.ServiceClasses(ns).Create(&v1beta1.ServiceClass{}); nil == e {
		return fmt.Errorf("needs a name (%s)", sc.Name)
	}
	if sp, e := scClient.ServicePlans(ns).Create(&v1beta1.ServicePlan{}); nil == e {
		return fmt.Errorf("needs a name (%s)", sp.Name)
	}
	return nil
}
...
func testClusterServiceBrokerClient(client servicecatalogclient.Interface, name string) error {
	...
	broker := &v1beta1.ClusterServiceBroker{ObjectMeta: metav1.ObjectMeta{Name: name}, Spec: v1beta1.ClusterServiceBrokerSpec{CommonServiceBrokerSpec: v1beta1.CommonServiceBrokerSpec{URL: "https://example.com"}}}
	...
	brokers, err = brokerClient.List(metav1.ListOptions{FieldSelector: "metadata.name=" + name})
	if err != nil {
		return fmt.Errorf("error listing ClusterServiceBrokers: %v", err)
	}
	if 1 != len(brokers.Items) {
		return fmt.Errorf("should have exactly one broker, had %v brokers", len(brokers.Items))
	}
	...
	if name != brokerServer.Name && broker.ResourceVersion == brokerServer.ResourceVersion {
		return fmt.Errorf("didn't get the same broker back from the server \n%+v\n%+v", broker, brokerServer)
	}
	brokerListed := &brokers.Items[0]
	if !reflect.DeepEqual(brokerServer, brokerListed) {
		return fmt.Errorf("Didn't get the same instance from list and get: diff: %v", diff.ObjectReflectDiff(brokerServer, brokerListed))
	}
	authSecret := &v1beta1.ObjectReference{Namespace: "test-namespace", Name: "test-name"}
	brokerServer.Spec.AuthInfo = &v1beta1.ClusterServiceBrokerAuthInfo{Basic: &v1beta1.ClusterBasicAuthConfig{SecretRef: authSecret}}
	brokerUpdated, err := brokerClient.Update(brokerServer)
	if nil != err || "test-namespace" != brokerUpdated.Spec.AuthInfo.Basic.SecretRef.Namespace || "test-name" != brokerUpdated.Spec.AuthInfo.Basic.SecretRef.Name {
		return fmt.Errorf("broker wasn't updated, %v, %v", brokerServer, brokerUpdated)
	}
	readyConditionTrue := v1beta1.ServiceBrokerCondition{Type: v1beta1.ServiceBrokerConditionReady, Status: v1beta1.ConditionTrue, Reason: "ConditionReason", Message: "ConditionMessage"}
	brokerUpdated.Status = v1beta1.ClusterServiceBrokerStatus{CommonServiceBrokerStatus: v1beta1.CommonServiceBrokerStatus{Conditions: []v1beta1.ServiceBrokerCondition{readyConditionTrue}}}
	...
	readyConditionFalse := v1beta1.ServiceBrokerCondition{Type: v1beta1.ServiceBrokerConditionReady, Status: v1beta1.ConditionFalse, Reason: "ConditionReason", Message: "ConditionMessage"}
	...
	brokerServer, err = brokerClient.Get(name, metav1.GetOptions{})
	if nil != err || "test-namespace" != brokerServer.Spec.AuthInfo.Basic.SecretRef.Namespace || "test-name" != brokerServer.Spec.AuthInfo.Basic.SecretRef.Name {
...
func testNamespacedServiceBrokerClient(client servicecatalogclient.Interface, namespace, name string) error {
	...
	broker := &v1beta1.ServiceBroker{ObjectMeta: metav1.ObjectMeta{Name: name}, Spec: v1beta1.ServiceBrokerSpec{CommonServiceBrokerSpec: v1beta1.CommonServiceBrokerSpec{URL: "https://example.com"}}}
	brokers, err := brokerClient.List(metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("error listing brokers (%s)", err)
	}
	...
	brokers, err = brokerClient.List(metav1.ListOptions{FieldSelector: "metadata.name=" + name})
	if err != nil {
		return fmt.Errorf("error listing ServiceBrokers: %v", err)
	}
	if 1 != len(brokers.Items) {
		return fmt.Errorf("should have exactly one broker, had %v brokers", len(brokers.Items))
	}
	brokers, err = brokerClient.List(metav1.ListOptions{FieldSelector: "metadata.namespace=" + namespace})
	if err != nil {
		return fmt.Errorf("error listing ServiceBrokers: %v", err)
	}
	if 1 != len(brokers.Items) {
	...
	authSecret := &v1beta1.LocalObjectReference{Name: "test-name"}
	brokerServer.Spec.AuthInfo = &v1beta1.ServiceBrokerAuthInfo{Basic: &v1beta1.BasicAuthConfig{SecretRef: authSecret}}
	brokerUpdated, err := brokerClient.Update(brokerServer)
	if nil != err || "test-name" != brokerUpdated.Spec.AuthInfo.Basic.SecretRef.Name {
		return fmt.Errorf("broker wasn't updated, %v, %v", brokerServer, brokerUpdated)
	}
	readyConditionTrue := v1beta1.ServiceBrokerCondition{Type: v1beta1.ServiceBrokerConditionReady, Status: v1beta1.ConditionTrue, Reason: "ConditionReason", Message: "ConditionMessage"}
	brokerUpdated.Status = v1beta1.ServiceBrokerStatus{CommonServiceBrokerStatus: v1beta1.CommonServiceBrokerStatus{Conditions: []v1beta1.ServiceBrokerCondition{readyConditionTrue}}}
	brokerUpdated.Spec.URL = "http://shouldnotupdate.com"
	brokerUpdated2, err := brokerClient.UpdateStatus(brokerUpdated)
	if nil != err || len(brokerUpdated2.Status.Conditions) != 1 {
		return fmt.Errorf("broker status wasn't updated")
	...
	brokerServer, err = brokerClient.Get(name, metav1.GetOptions{})
	if nil != err || "test-name" != brokerServer.Spec.AuthInfo.Basic.SecretRef.Name {
		return fmt.Errorf("broker wasn't updated (%v)", brokerServer)
	}
	if e, a := readyConditionFalse, brokerServer.Status.Conditions[0]; !reflect.DeepEqual(e, a) {
		return fmt.Errorf("Didn't get matching ready conditions:\nexpected: %v\n\ngot: %v", e, a)
...
func testClusterServiceClassClient(client servicecatalogclient.Interface, name string) error {
	...
	serviceClass := &v1beta1.ClusterServiceClass{ObjectMeta: metav1.ObjectMeta{Name: name}, Spec: v1beta1.ClusterServiceClassSpec{ClusterServiceBrokerName: "test-broker", CommonServiceClassSpec: v1beta1.CommonServiceClassSpec{Bindable: true, ExternalName: name, ExternalID: "b8269ab4-7d2d-456d-8c8b-5aab63b321d1", Description: "test description"}}}
	...
	if len(serviceClasses.Items) > 0 {
		return fmt.Errorf("serviceClasses should not exist on start, had %v serviceClasses", len(serviceClasses.Items))
	}
	...
	if name != serviceClassAtServer.Name {
		return fmt.Errorf("didn't get the same ClusterServiceClass back from the server \n%+v\n%+v", serviceClass, serviceClassAtServer)
	}
	serviceClasses, err = serviceClassClient.List(metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("error listing service classes (%s)", err)
	...
	if serviceClassAtServer.Name != name && serviceClass.ResourceVersion == serviceClassAtServer.ResourceVersion {
		return fmt.Errorf("didn't get the same ClusterServiceClass back from the server \n%+v\n%+v", serviceClass, serviceClassAtServer)
	}
	serviceClassListed := &serviceClasses.Items[0]
	if !reflect.DeepEqual(serviceClassAtServer, serviceClassListed) {
		return fmt.Errorf("Didn't get the same instance from list and get: diff: %v", diff.ObjectReflectDiff(serviceClassAtServer, serviceClassListed))
	}
	...
	serviceClass2 := &v1beta1.ClusterServiceClass{ObjectMeta: metav1.ObjectMeta{Name: sc2Name}, Spec: v1beta1.ClusterServiceClassSpec{ClusterServiceBrokerName: "test-broker", CommonServiceClassSpec: v1beta1.CommonServiceClassSpec{Bindable: true, ExternalName: sc2Name, ExternalID: sc2ID, Description: "test description 2"}}}
...
func TestNamespacedServiceClassClient(t *testing.T) {
	...
	defer shutdownServer()
	if err := testNamespacedServiceClassClient(client, namespace, name); err != nil {
		t.Fatal(err)
	}
}
func testNamespacedServiceClassClient(client servicecatalogclient.Interface, namespace, name string) error {
	...
	serviceClass := &v1beta1.ServiceClass{ObjectMeta: metav1.ObjectMeta{Name: name}, Spec: v1beta1.ServiceClassSpec{ServiceBrokerName: "test-broker", CommonServiceClassSpec: v1beta1.CommonServiceClassSpec{Bindable: true, ExternalName: name, ExternalID: "b8269ab4-7d2d-456d-8c8b-5aab63b321d1", Description: "test description"}}}
	serviceClasses, err := serviceClassClient.List(metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("error listing service classes (%s)", err)
	}
	...
	if name != serviceClassAtServer.Name {
		return fmt.Errorf("didn't get the same ServiceClass back from the server \n%+v\n%+v", serviceClass, serviceClassAtServer)
	}
	serviceClasses, err = serviceClassClient.List(metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("error listing service classes (%s)", err)
	...
	sc2Name := name + "2"
	sc2ID := "someotheridhere"
	serviceClass2 := &v1beta1.ServiceClass{ObjectMeta: metav1.ObjectMeta{Name: sc2Name}, Spec: v1beta1.ServiceClassSpec{ServiceBrokerName: "test-broker", CommonServiceClassSpec: v1beta1.CommonServiceClassSpec{Bindable: true, ExternalName: sc2Name, ExternalID: sc2ID, Description: "test description 2"}}}
	_, err = serviceClassClient.Create(serviceClass2)
	if nil != err {
	...
	serviceClasses, err = serviceClassClient.List(metav1.ListOptions{FieldSelector: "spec.serviceBrokerName=" + "test-broker"})
	if err != nil {
		return fmt.Errorf("error listing service classes (%s)", err)
	}
	if 2 != len(serviceClasses.Items) {
	...
	serviceClasses, err = serviceClassClient.List(metav1.ListOptions{FieldSelector: "metadata.name=" + name})
	if err != nil {
		return fmt.Errorf("error listing ServiceClasses: %v", err)
	}
	if 1 != len(serviceClasses.Items) {
		return fmt.Errorf("should have exactly one ServiceClass, had %v ServiceClasses", len(serviceClasses.Items))
	}
	serviceClasses, err = serviceClassClient.List(metav1.ListOptions{FieldSelector: "metadata.namespace=" + namespace})
	if err != nil {
		return fmt.Errorf("error listing ServiceClasses: %v", err)
	}
	if 2 != len(serviceClasses.Items) {
...
func testClusterServicePlanClient(client servicecatalogclient.Interface, name string) error {
	...
	bindable := true
	servicePlan := &v1beta1.ClusterServicePlan{ObjectMeta: metav1.ObjectMeta{Name: name}, Spec: v1beta1.ClusterServicePlanSpec{ClusterServiceBrokerName: "test-broker", CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{Bindable: &bindable, ExternalName: name, ExternalID: "b8269ab4-7d2d-456d-8c8b-5aab63b321d1", Description: "test description"}, ClusterServiceClassRef: v1beta1.ClusterObjectReference{Name: "test-serviceclass"}}}
	...
	if len(servicePlans.Items) > 0 {
		return fmt.Errorf("servicePlans should not exist on start, had %v servicePlans", len(servicePlans.Items))
	}
	...
	if name != servicePlanAtServer.Name {
		return fmt.Errorf("didn't get the same ClusterServicePlan back from the server \n%+v\n%+v", servicePlan, servicePlanAtServer)
	}
	servicePlans, err = servicePlanClient.List(metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("error listing service plans (%s)", err)
	...
	if servicePlanAtServer.Name != name && servicePlan.ResourceVersion == servicePlanAtServer.ResourceVersion {
		return fmt.Errorf("didn't get the same ClusterServicePlan back from the server \n%+v\n%+v", servicePlan, servicePlanAtServer)
	}
	...
	sp2Name := name + "2"
	sp2ID := "anotheridhere"
	servicePlan2 := &v1beta1.ClusterServicePlan{ObjectMeta: metav1.ObjectMeta{Name: sp2Name}, Spec: v1beta1.ClusterServicePlanSpec{ClusterServiceBrokerName: "test-broker", CommonServicePlanSpec: v1beta1.CommonServicePlanSpec{Bindable: &bindable, ExternalName: sp2Name, ExternalID: sp2ID, Description: "test description 2"}, ClusterServiceClassRef: v1beta1.ClusterObjectReference{Name: "test-serviceclass"}}}
...
func TestNamespacedServicePlanClient(t *testing.T) {
	...
	defer shutdownServer()
	if err := testNamespacedServicePlanClient(client, namespace, name); err != nil {
		t.Fatal(err)
	}
}
func testNamespacedServicePlanClient(client servicecatalogclient.Interface, namespace, name string) error {
	...
	if name != servicePlanAtServer.Name {
		return fmt.Errorf("didn't get the same ServicePlan back from the server \n%+v\n%+v", servicePlan, servicePlanAtServer)
	}
	servicePlans, err = servicePlanClient.List(metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("error listing service plans (%s)", err)
	...
	servicePlans, err = servicePlanClient.List(metav1.ListOptions{FieldSelector: "metadata.name=" + name})
	if err != nil {
		return fmt.Errorf("error listing ServicePlans: %v", err)
	}
	if 1 != len(servicePlans.Items) {
		return fmt.Errorf("should have exactly one ServicePlan, had %v ServicePlans", len(servicePlans.Items))
	}
	servicePlans, err = servicePlanClient.List(metav1.ListOptions{FieldSelector: "metadata.namespace=" + namespace})
	if err != nil {
		return fmt.Errorf("error listing ServicePlans: %v", err)
	}
	if 2 != len(servicePlans.Items) {
		return fmt.Errorf("should have exactly two ServicePlans, had %v ServicePlans", len(servicePlans.Items))
	}
	err = servicePlanClient.Delete(name, &metav1.DeleteOptions{})
	if nil != err {
		return fmt.Errorf("serviceplan should be deleted (%s)", err)
...
func testInstanceClient(client servicecatalogclient.Interface, name string) error {
	...
	instance := &v1beta1.ServiceInstance{ObjectMeta: metav1.ObjectMeta{Name: name}, Spec: v1beta1.ServiceInstanceSpec{PlanReference: v1beta1.PlanReference{ClusterServiceClassExternalName: "service-class-name", ClusterServicePlanExternalName: "plan-name"}, ClusterServiceClassRef: &v1beta1.ClusterObjectReference{Name: "test-serviceclass"}, ClusterServicePlanRef: &v1beta1.ClusterObjectReference{Name: "test-serviceplan"}, Parameters: &runtime.RawExtension{Raw: []byte(instanceParameter)}, ExternalID: osbGUID}, Status: v1beta1.ServiceInstanceStatus{DeprovisionStatus: v1beta1.ServiceInstanceDeprovisionStatusNotRequired}}
	...
	if len(instances.Items) > 0 {
		return fmt.Errorf("instances should not exist on start, had %v instances", len(instances.Items))
	}
	...
	if instanceServer.Name != name && instanceServer.ResourceVersion == instance.ResourceVersion && instanceServer.Spec.ExternalID != osbGUID {
		return fmt.Errorf("didn't get the same instance back from the server \n%+v\n%+v", instance, instanceServer)
	}
	...
	readyConditionTrue := v1beta1.ServiceInstanceCondition{Type: v1beta1.ServiceInstanceConditionReady, Status: v1beta1.ConditionTrue, Reason: "ConditionReason", Message: "ConditionMessage"}
	instanceServer.Status = v1beta1.ServiceInstanceStatus{ReconciledGeneration: instanceServer.Generation, ObservedGeneration: instanceServer.Generation, ProvisionStatus: v1beta1.ServiceInstanceProvisionStatusProvisioned, Conditions: []v1beta1.ServiceInstanceCondition{readyConditionTrue}, DeprovisionStatus: v1beta1.ServiceInstanceDeprovisionStatusNotRequired}
...
func testBindingClient(client servicecatalogclient.Interface, name string) error {
	...
	binding := &v1beta1.ServiceBinding{ObjectMeta: metav1.ObjectMeta{Name: "test-binding"}, Spec: v1beta1.ServiceBindingSpec{InstanceRef: v1beta1.LocalObjectReference{Name: "bar"}, Parameters: &runtime.RawExtension{Raw: []byte(bindingParameter)}, ExternalID: "UUID-string"}, Status: v1beta1.ServiceBindingStatus{UnbindStatus: v1beta1.ServiceBindingUnbindStatusNotRequired}}
	...
	if name != bindingServer.Name {
		return fmt.Errorf("didn't get the same binding back from the server \n%+v\n%+v", binding, bindingServer)
	}
	if bindingServer.Spec.SecretName != "test-binding" {
		return fmt.Errorf("didn't get the right secret name back from the server \n%+v\n%+v", "test-binding", bindingServer.Spec.SecretName)
	}
	bindings, err = bindingClient.List(metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("error listing bindings (%s)", err)
	...
	if bindingServer.Name != name && bindingServer.ResourceVersion == binding.ResourceVersion {
		return fmt.Errorf("didn't get the same binding back from the server \n%+v\n%+v", binding, bindingServer)
	}
	bindingListed := &bindings.Items[0]
	if !reflect.DeepEqual(bindingListed, bindingServer) {
		return fmt.Errorf("Didn't get the same binding from list and get: diff: %v", diff.ObjectReflectDiff(bindingListed, bindingServer))
	}
	parameters := bpStruct{}
	...
	readyConditionTrue := v1beta1.ServiceBindingCondition{Type: v1beta1.ServiceBindingConditionReady, Status: v1beta1.ConditionTrue, Reason: "ConditionReason", Message: "ConditionMessage"}
	bindingServer.Status = v1beta1.ServiceBindingStatus{Conditions: []v1beta1.ServiceBindingCondition{readyConditionTrue}, UnbindStatus: v1beta1.ServiceBindingUnbindStatusNotRequired}
	...
	if bindingDeleted, err := bindingClient.Get(name, metav1.GetOptions{}); err == nil {
		return fmt.Errorf("binding should be deleted after finalizers cleared. got binding %#v", *bindingDeleted)
	}
	return nil
}
